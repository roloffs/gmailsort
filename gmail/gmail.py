...
def synchronize(creds, profile_name):
    ...
    if os.path.exists(userdata_path):
        ...
        if history_id > userdata.history_id:
            (history_items, err) = gmail_api.get_history_items(creds, userdata.history_id)
            if err:
                return (None, True)
            ...
            for history_item in history_items:
                messages_updated_ids.update(set(map(get_message_id, history_item.get('messagesAdded', []))))
                messages_updated_ids.update(set(map(get_message_id, history_item.get('labelsAdded', []))))
                messages_updated_ids.update(set(map(get_message_id, history_item.get('labelsRemoved', []))))
                messages_deleted_ids.update(set(map(get_message_id, history_item.get('messagesDeleted', []))))
            # Do not fetch messages that will be deleted anyway
            messages_updated_ids = messages_updated_ids.difference(messages_deleted_ids)
            (messages_updated, err) = gmail_api.get_messages(creds, messages_updated_ids)
            if err:
                return (None, True)
            for message in messages_updated:
                userdata.messages[message['id']] = message
            for message_id in messages_deleted_ids:
                # SPAM or TRASH messages are not stored in userdata, but can occur in history items
                if message_id in userdata.messages:
                    del userdata.messages[message_id]
            userdata.history_id = history_id

    else:
        print(f'No local database found: download all messages from gmail [create \'{userdata_path}\']')
...
def __label_exists(label_name, labels):
    ...
    for label in labels.values():
        if label_name_tokens == list(filter(None, label['name'].lower().split('/'))):
            return True
    return False
...
def partition_messages_by_sender_domain(userdata, dst_label_name):
    ...
    if dst_label_name:
        print(f'Analyze sender email addresses of {len(messages)} messages from \'{dst_label_name}\'')
    else:
    ...
    for message in messages:
        ...
        if sender_address:
            ...
            print(f'No sender address found in message, ignore:\n{json.dumps(message, indent=2, ensure_ascii=False, sort_keys=True)}')
    ...
    for label_name in label_names:
        ...
        for label in labels:
            ...
            if last_component == label_name.lower():
                ...
                print(f'Found unexact label match for \'{label_name.lower()}\': \'{label["name"]}\'')

    return found_labels


def partition_messages_by_prefix(userdata, messages, dst_label_name):
    # Create dict with label ids (strings) as keys and lists of messages as values
    # (the empty string as key is a collector for messages with invalid label matches)
    labels = userdata.labels
    label_ids = dict()
    for message in messages:
        msg_labels = __get_message_labels_by_prefix(message, labels, dst_label_name)
...
def modify_message_labels(creds, messages, add_label_ids, remove_label_ids):
    ...
    return gmail_api.modify_message_labels(creds, message_ids, add_label_ids, remove_label_ids)
